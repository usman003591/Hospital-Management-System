...
class MedicineRoute extends Model
{
    ...
    public static function getAll()
    {
        ...
        if ($search['status'] !== false) {
            if ($search['status'] == 1) {
                $data = $data->where('medicine_routes.status', 1);
            } elseif ($search['status'] == 0) {
                $data = $data->where('medicine_routes.status', 0);
            }
        }
    ...
    public function addForm($request = false)
    {
        ...
        session()->flash('success', 'Medicine route created successfully');
        return Redirect::route('medicine_routes.index');
    }
    ...
    public function updateForm($request = false)
    {
        ...
        $obj = MedicineRoute::find($request->id);
        ...
        session()->flash('success', 'Medicine route updated successfully');
        return Redirect::route('medicine_routes.index');
    }
    ...
    public function deleteObj()
    {
        ...
        return response()->json([
            'status' => 200,
            'message' => 'Medicine route has been deleted successfully',
        ]);
    }
}
