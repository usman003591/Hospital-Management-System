...
class MedicineInventoryStatus extends Model
{
    ...
    protected $table = 'pos_medicine_inventory_statuses';
    ...
    public static function getAll()
    {
        ...
        if ($search['status'] !== false) {
            if ($search['status'] == 1) {
                $data = $data->where('pos_medicine_inventory_statuses.status', 1);
            } elseif ($search['status'] == 0) {
                $data = $data->where('pos_medicine_inventory_statuses.status', 0);
            }
        }
    ...
    public function addForm($request = false)
    {
        ...
        session()->flash('success', 'Medicine inventory status created successfully');
        return Redirect::route('medicine_inventory_statuses.index');
    }
    ...
    public function updateForm($request = false)
    {
        ...
        $obj = MedicineInventoryStatus::find($request->id);
        ...
        session()->flash('success', 'Medicine inventory status updated successfully');
        return Redirect::route('medicine_inventory_statuses.index');
    }
    ...
    public function deleteObj()
    {
        ...
        return response()->json([
            'status' => 200,
            'message' => 'Medicine inventory status has been deleted successfully',
        ]);
    }
}
