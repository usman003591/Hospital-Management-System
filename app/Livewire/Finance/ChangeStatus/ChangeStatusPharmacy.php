...
class ChangeStatusPharmacy extends Component
{
    public $invoiceId;
    public $changed_at;
    public $remarks;
    ...
    public $old_value;
    protected $listeners = ['refreshPharmacyFinanceStatus' => 'render'];
    protected $rules = [
    ...
    public function mount($invoiceId)
    {
        ...
        $log = FinanceVerificationAuditLog::where('verifiable_type_id', $invoiceId)
            ->where('verifiable_type', 'pos_invoices')
            ->latest()
            ->first();
        if ($log) {
        ...
        public function submit()
       {
        ...
        $item = PosInvocie::find($this->invoiceId);
        if ($item) {
            ...
            if($verifiedObj) {
                $this->dispatch('show-toast', type: 'success', message: 'Pharmacy invoice status changed successfully!');
                $this->selectedItems = [];
            } else {
                $this->dispatch('show-toast', type: 'error', message: 'Failed to verify pharmacy invoice!');
            }
        }
    ...
    {
        return view('livewire.finance.change-status.change-status-pharmacy');
    }
}
