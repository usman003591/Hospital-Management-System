...
class AllOpdsListing extends Component
{
    use WithPagination;
    public $q = '';
    public $status = '';
    ...
    public function render()
    {
        ...
        $data = ClinicalDiagnosis::leftJoin('patients as p', 'p.id', 'clinical_diagnoses.patient_id')
            ...
            ->when($this->hospital !== '', fn($query) =>
                $query->where('clinical_diagnoses.hospital_id', (int) $this->hospital))
            ->orderBy('clinical_diagnoses.created_at', 'desc')
            ->paginate(10);


        return view('livewire.clinical-diagnosis.opd.all-opds-listing', [
            'data' => $data,
            'hospitals' => $hospitals,
