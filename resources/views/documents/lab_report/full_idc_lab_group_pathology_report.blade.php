...
<body>
    ...
    // $qrCode =
    base64_encode(file_get_contents('https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=DummyQR'));
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="patient-info" style="padding-top : 10px">
                <div>@isset($patient->name_of_patient) {{ \Illuminate\Support\Str::limit($patient->name_of_patient, 15,
                    '..') }} @endisset </div>
                ...
                <div>Ref.By:<strong> @isset($doctor->doctor_name) {{
                        \Illuminate\Support\Str::limit($doctor->doctor_name, 15, '..') }} @else self @endisset </strong>
                </div>
            </td>
    ...
    @isset($processedData['tests'])
    @foreach ($processedData['tests'] as $investigation)

    @php

    $investigationId = $investigation->investigation_id;
    $patientId = $investigation->patient_id;

    $resultsData = null;
    $allParameters = null;
    $pivotedResults = null;

    $resultsData = \App\Models\LabGroupTest::with('results')
    ...
    <table class="results-table">
        <thead>
            <tr style="background: #ccc; font-weight: bold;">
                <th width="20%"></th>
                @for($i = 0; $i < 4; $i++) @if(isset($resultsData[$i])) <th>{{
                    \Carbon\Carbon::parse($resultsData[$i]->report_date)->format('d M Y') }}</th>
                    @endif
                    @endfor
                    {{-- <th>8-Sep-25</th>
                    <th>3-Jan-25</th>
    ...
    <div class="comments">
        ...
        <em style="font-weight: bold">Disclaimer:</em> Every diagnostic test has scientific acceptable technology or
        technique based limitations of uncertainty of measurement, false positive or false negative and so do not fall
        under the domain of negligence. In case of any such scenario, we offer free repeat of test within 24-48 hours.
    </div>
