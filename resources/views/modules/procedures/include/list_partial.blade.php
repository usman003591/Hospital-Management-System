                        ...
                        <thead class="text-gray-500 fs-7 text-uppercase">
                            <tr role="row" class="text-gray-800 border-gray-200 fw-semibold fs-6 border-bottom">
                                <th class="text-gray-800 border-gray-200 fw-semibold fs-6 border-bottom"
                                    ...
                                    tabindex="0" style="min-width: 14rem"><span class="dt-column-title">Name</span><span class="dt-column-order"></span></th>
                                    ...
                                    style="min-width: 10rem"><span class="dt-column-title">Created at</span><span
                                ...
                                <th class="dt-orderable-asc dt-orderable-desc" data-dt-column="3" rowspan="1"
                                    colspan="1" aria-label="Status: Activate to sort" tabindex="0"
                                    style="min-width: 7rem"><span class="dt-column-title">Verification Status</span><span
                                        class="dt-column-order"></span></th>
                                <th class="dt-orderable-asc dt-orderable-desc" data-dt-column="3" rowspan="1"
                                    colspan="1" aria-label="Status: Activate to sort" tabindex="0"
                                    style="min-width: 7rem"><span class="dt-column-title">Is Manual</span><span
                                        class="dt-column-order"></span></th>
                         ...
                         <tbody>
                            @forelse ($data as $obj)
                                <tr>
                                    ...
                                    <td>
                                        <select @if($obj->verification_status=='approved') class="form-select form-select-solid w-125px bg-light-success text-success" @endif
                                        @if($obj->verification_status=='pending') class="form-select form-select-solid w-125px bg-light-warning text-warning" @endif
                                        @if($obj->verification_status=='rejected') class="form-select form-select-solid w-125px bg-light-danger text-danger" @endif data-control="select2" data-dropdown-css-class="w-200px" id="verificationStatusSelect" data-placeholder="Select verification status" data-hide-search="true" data-id="{{ $obj->id }}">

                                            <option class="bg-light-success text-success" value="approved" @if($obj->verification_status=='approved') selected @endif > Approved </option>
                                            <option class="bg-light-warning text-warning" value="pending" @if($obj->verification_status=='pending') selected @endif > Pending </option>
                                            <option class="bg-light-danger text-danger" value="rejected" @if($obj->verification_status=='rejected') selected @endif > Rejected </option>

                                        </select>
                                        {{-- {{ ucfirst($obj->verification_status) }} --}}
                                    </td>
                                    <td>
                                        @if($obj->is_manual == 1)
                                    ...
                                    <td class="align-middle text-end">
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            <form action="{{ route($page.'.edit', ['id' => $obj->id]) }}" method="GET" style="display: inline;">
                                                @csrf
                                            ...
                                            <form action="{{ route($page.'.delete', ['id' => $obj->id]) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" data-bs-toggle="modal" data-bs-target="#kt_modal_delete_{{$page}}" data-id="{{ $obj->id }}"
