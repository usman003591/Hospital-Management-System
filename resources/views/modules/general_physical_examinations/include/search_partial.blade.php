...
<div class="mb-5 form-group row bg-search">
    ...
    <div class="col-lg-3">
        <select id="parent_id" class="form-select ajax_call_trigger form-select-solid bg-body-secondary"
            ...
            @foreach($ParentPhysicalExaminations as $PhysicalExamination)
            <option value="{{ $PhysicalExamination->id }}" {{ isset($search['parent_id']) &&
                (string)$search['parent_id']===(string)$PhysicalExamination->id ? 'selected' : '' }}>
