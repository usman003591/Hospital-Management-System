...
<div class="mb-5 form-group row bg-search">
    ...
    <div class="col-lg-2">
        <select id="floor" name="floor" class="form-select ajax_call_trigger form-select-solid bg-body-secondary"
            data-kt-select2="true" data-placeholder="Select Floor" data-allow-clear="false">
            <option selected disabled>{{ __('Select Floor')}}</option>
            <option value="0">All Floors</option>
            @foreach($floors as $floor)
                <option value="{{ $floor->id }}" {{ isset($search['floor']) && $search['floor'] == $floor->id ? 'selected' : '' }}>
                    {{ $floor->floor_name }}
                </option>
            @endforeach
        </select>
