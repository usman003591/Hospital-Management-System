...
<div class="mb-5 form-group row bg-search">
    ...
    <div class="col-lg-3">
        <select id="investigation_type" name="investigation_type"
            ...
            data-placeholder="Select Investigation Type" data-allow-clear="false">
            <option selected disabled>{{ __('Select Investigation Type') }}</option>
            <option value="">All Investigation Types</option>
            @foreach($types as $type)
            <option value="{{ $type->id }}" {{ isset($search['types']) && $search['types']==$type->id ? 'selected' : ''
                }}>
                {{ $type->name }}
            </option>
            @endforeach
