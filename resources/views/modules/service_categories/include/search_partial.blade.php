...
<div class="mb-5 form-group row bg-search">
    ...
    <div class="col-lg-3">
        <select id="parent_id" class="form-select ajax_call_trigger form-select-solid bg-body-secondary"
            data-kt-select2="true" data-placeholder="Filter By Parent" data-allow-clear="true">
            <option value="">All Categories</option>
            <option value="0" {{ isset($search['parent_id']) && $search['parent_id']==='0' ? 'selected' : '' }}>Parent
                Categories Only</option>
            @foreach($parentCategories as $category)
            <option value="{{ $category->id }}" {{ isset($search['parent_id']) &&
                (string)$search['parent_id']===(string)$category->id ? 'selected' : '' }}>
