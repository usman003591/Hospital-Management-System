...
<div class="mb-5 form-group row bg-search">
    ...
    <div class="col-lg-2">
        <select id="doctor" class="form-select ajax_call_trigger form-select-solid bg-body-secondary"
            data-kt-select2="true" data-placeholder="Select doctor" data-allow-clear="false">
            <option disabled> {{ __('Select doctor') }}</option>
            <option selected value="0">All</option>
            @foreach ($doctors as $item)
                <option value="{{ $item->id }}" {{ isset($search['doctor']) && $search['doctor'] == $item->id }}>
                    {{ $item->doctor_name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-lg-2">
        <select id="status" class="form-select ajax_call_trigger form-select-solid bg-body-secondary"
            ...
            <option value="approved" {{ isset($search['status']) && $search['status'] == 'approved' }}>Approved
            </option>
            <option value="cancelled" {{ isset($search['status']) && $search['status'] == 'cancelled' }}>Cancelled
            </option>
        </select>
    </div>
